...
if [ "$#" -ne 1 ]; then
  echo "Usage: ./scripts/disable-local-admin.sh admin@example.com" >&2
  exit 64
fi
...
if [ -n "$DOCKER_BIN" ]; then
  ...
  if [ -n "$BACKEND_CONTAINER" ]; then
    "$DOCKER_BIN" compose -f "$ROOT_DIR/docker-compose.local.yml" exec -T backend python -m local_app.admin_cli disable "$1"
    exit $?
  fi
fi
...
PYTHONPATH="$ROOT_DIR/backend/src" "$PYTHON_BIN" -m local_app.admin_cli disable "$1"
